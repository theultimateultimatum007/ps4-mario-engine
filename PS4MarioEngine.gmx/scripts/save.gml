...
**
**  Given:
**      file    The location of the save file (bare filename).
**
**  Returns:
...
// PS4: some runners only accept bare filenames in savedata
if (file == -1 && scr_is_ps4() && path != argument[0])
    file = file_text_open_write(argument[0]);

if (file == -1) {

    if (scr_is_ps4() && instance_exists(obj_persistent))
        obj_persistent.message = "Save failed";
    return false;
}
